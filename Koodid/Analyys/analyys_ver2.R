...
#ESA lapsehooldusel isad ---------------
#mitteaktiivsed mehed 24-49
source("https://raw.githubusercontent.com/AndresVork/rsdmx_esa/master/rsdmx_esa_fn.R")
tempmehed <- rsdmx_esa("TT462")
mehed <- tempmehed %>%  
  filter(DIM3label.et=="Mehed", DIM4label.et == "25-49", DIM5label.et=="Mitteaktiivsed, tuhat", obsTime>=2004) %>% 
  mutate(kpv = as.Date(paste0(obsTime, "-", as.numeric(as.roman(DIM2))*3, "-01"))) %>% 
  arrange(kpv) %>% 
  select(kpv, obsTime, DIM2, obsValue)

ggplot(mehed, aes(x=kpv, y = obsValue)) +
  geom_line() +
  geom_smooth()
